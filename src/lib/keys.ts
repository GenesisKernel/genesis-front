...
export const generateKey = () => {
    return randomEngine.hex(64, false);
};

export const generatePassphrase = (count: number = 15) => {
    const result: string[] = [];
...
export const binToHex = (input: string) => {
    let result = '';
    let char = '';

    for (let i = 0; i < input.length; i++) {
        char = input.charCodeAt(i).toString(16);
...
export const hexToBin = (input: string) => {
    let result = '';

    for (let i = 0; i < input.length; i += 2) {
        let byte = parseInt(input.substr(i, 2), 16);
